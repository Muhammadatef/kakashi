...
<img src="./logo.svg" width="130" height="130" alt="Kakashi"/>
...
┌─────────────────────────────────────────────────────┐
│  TRADITIONAL FLOW                                   │
│  You → [paste file] → AI Agent → External LLM  ⚠️  │
│                                                     │
│  WITH KAKASHI                                       │
...
> ✓ `/kakashi-scan /path/to/sample_data.csv`<br/>
> ✗ `/kakashi-scan @sample_data.csv`<br/>
...
> **Rule 2 — `/kakashi-scan` is agent-safe; `/kakashi-audit` is verbose by design.**<br/>
> `/kakashi-scan` runs with `--quiet` and emits only counts (`17 findings (0 id · 2 personal info · 15 credentials)`). No secret previews enter the agent's context.<br/>
...
**1. Scan** — see what's sensitive before it moves.
...
```
✓  Masked → masked_quarterly_report.xlsx
   6 replacements  (3 credentials · 3 personal info)
   Original untouched.
```
...
/kakashi-scan <path>  scan only, counts only (--quiet, agent-safe)
...
|-------|:--------------:|:--------------:|---------|
| **Claude Code** | ✓ | full set (6) | `--only claude` |
| **Cursor** | ✓ | full set (6) | `--only cursor` |
| **OpenAI Codex** | ✓ | full set (6) | `--only codex` |
| **Windsurf** | ✓ | full set (6) | `--only windsurf` |
| **Cline** | ✓ | full set (6) | `--only cline` |
| **GitHub Copilot** | ✓ | via `.github/copilot-instructions.md` | `--only copilot` |
| **Continue** | ⚡ per session | `/kakashi` | `--only continue` |
| **Aider** | ⚡ per session | `/kakashi` | `--only aider` |
| **Roo Code** | ⚡ per session | `/kakashi` | `--only roo` |
| **Kilo Code** | ⚡ per session | `/kakashi` | `--only kilo` |
| **OpenHands** | ⚡ per session | `/kakashi` | `--only openhands` |
| **Warp** | ⚡ per session | `/kakashi` | `--only warp` |
| **Replit Agent** | ⚡ per session | `/kakashi` | `--only replit` |
| **Augment Code** | ⚡ per session | `/kakashi` | `--only augment` |
| **JetBrains Junie** | ⚡ per session | `/kakashi` | `--only junie` |

> ✓ = always on, activates from first message<br/>
> ⚡ = type `/kakashi` once per session to activate
...
| Format | Read | Mask | Reconstruct |
|--------|:----:|:----:|:-----------:|
| Word `.docx` | ✓ | ✓ | full `.docx` |
| PowerPoint `.pptx` | ✓ | ✓ | full `.pptx` |
| Excel `.xlsx` `.xls` | ✓ | ✓ | full `.xlsx` |
| PDF `.pdf` | ✓ | ✓ | masked `.txt`* |
| CSV / TSV | ✓ | ✓ | same format |
| JSON / JSONL / JSON5 | ✓ | ✓ | same format |
| YAML / TOML | ✓ | ✓ | same format |
| XML | ✓ | ✓ | same format |
| Markdown | ✓ | ✓ | same format |

> *PDF reconstruction requires server-side rendering. Kakashi outputs a clean masked `.txt`.
...
Claude: Before I look at it, let me scan it first.
        ...
        🔑 0 credentials  ·  👤 12 personal records
...
kakashi: 🔑 3 credentials found
         db_password: "Pr0d_P@55w0rd!"    →  [ENV_SECRET_1]
         api_key: "sk-proj-aBcDeF..."     →  [OPENAI_KEY_1]
         jwt_secret: "a1b2c3d4e5f6..."    →  [HEX_SECRET_1]

         Run /kakashi-mask to apply.
```
...
# ⚡ 1 credential found — JWT_SECRET — masking before sending
...
Flags:
  ...
  -q, --quiet                  Counts only — no secret previews in stdout
                               (use this when an AI agent invokes kakashi)
...
|------|:---:|:---:|:---:|:---:|:---:|
| **Kakashi** | **✓ 20+ agents** | **✓** | **✓** | **✓** | **✓** |
| GitLeaks | ✗ | ✓ | ✗ detect-only | ✗ | partial |
| TruffleHog | ✗ | ✓ | ✗ detect-only | ✗ | partial |
| detect-secrets (Yelp) | ✗ | ✓ | ✗ detect-only | ✗ | partial |
| Microsoft Presidio | ✗ (Python SDK) | ✓ | ✓ | ✗ text only | ✗ heavy stack |
| AWS Comprehend / Macie | ✗ | ✗ cloud | ✓ | ✗ | ✗ |
| Google DLP / Azure PII | ✗ | ✗ cloud | ✓ | ✗ | ✗ |
| Skyflow / Tonic Textual | ✗ | ✗ cloud | ✓ | partial | ✗ |
| `redact-pii` / `pii-redact` (npm) | ✗ | ✓ | partial | ✗ | ✓ |
| Generic Cursor/Claude rules | ✓ but DIY | ✓ | ✗ no engine | ✗ | ✗ |
...
**kakashi** · MIT · built with 🥷 by [Mohamed Atef Fahmy](https://github.com/Muhammadatef) · [LinkedIn](https://www.linkedin.com/in/mohamed-atef-fahmy-75475a125/)
...
If kakashi saved your credentials today — ⭐ it costs nothing.
